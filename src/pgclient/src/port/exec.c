...
validate_exec(const char *path)
{
	...
	{
		strncpy(path_exe, path, sizeof(path_exe));
		strncat(path_exe, ".exe", sizeof(path_exe)-strlen(path_exe));
		path = path_exe;
	}
#endif
...
resolve_symlinks(char *path)
{
	...
	for (;;)
	{
		...
		link_buf[rllen] = '\0';
		strncpy(path, link_buf, strlen(link_buf) + 1);
	}

	/* must copy final component out of 'path' temporarily */
	strncpy(link_buf, fname, sizeof(link_buf));
